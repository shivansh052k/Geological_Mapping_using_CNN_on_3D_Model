...
parser.add_argument('--max_epoch', type=int, default=175) #nombre d'époques
parser.add_argument('--start_epoch', type=int, default=0)

parser.add_argument('--color_aug', type=float,
                    default=0.25, help='level of color augmentation')
parser.add_argument('--rotate', type=float,
                    default=45, help='rotation in degree')
parser.add_argument('--shear', type=float,
                    default=0.4, help='shearing in radians')
parser.add_argument('--resize', type=float,
                    default=0.2, help='recize ratio around 1')
...
parser.add_argument('--gscnn_roots', default= 'Colab Notebooks/GSCNN-master',
                    help='minimum testing (1 epoch run ) to verify nothing failed')
...
def main():
    ...
    #importer les données pour l'entrainement
    ...
    df = pd.DataFrame(columns = ['epoch', 'train_loss', 'validate'])
	...
	# Update EPOCH CTR
        ...
        #on entraine pour une époque
        train_loss = train(train_loader, net, criterion, optim, epoch, writer)
              
        #on valide
        val_loss, IOU = validate(val_loader, net, criterion_val,
                 optim, epoch, writer)
        
        #on stock les informations du loss dans un dataframe
        df_temp = pd.DataFrame([[epoch, train_loss, val_loss,  IOU]],
                               columns = ['epoch', 'train_loss', 'validate', 'IOU'])
        df = df.append(df_temp)
        df.to_pickle('{0}results/history_df.pkl'.format(cfg.DATASET.CITYSCAPES_DIR))
...
def train(train_loader, net, criterion, optimizer, curr_epoch, writer):
    ...
    for i, data in enumerate(train_loader):
        ...
        #on fait la backpropagation sur le réseau à partir des loss
...
def validate(val_loader, net, criterion, optimizer, curr_epoch, writer):
    ...
    for vi, data in enumerate(val_loader):
        ...
        #Image Dumps
        if vi < 10:
            dump_images.append([mask, seg_predictions, img_names, input.cpu()])
...
def evaluate(val_loader, net):
    ...
    for thresh in args.eval_thresholds.split(','):
        ...
        for vi, data in enumerate(val_loader):
            ...
            #add noise to inputs
            # input += torch.randn(input.size()) * random.uniform(0, 0.03) #+0
